	...
	private void initPlayLayout() {
		...
		play_pauseButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (isLoading) {
					play_pauseButton
							.setImageResource(R.mipmap.play_btn_pause_big_detail_down);
					return;
				}
				if (mMediaPlayerDelegate.isPlaying()) {
					...
					if (!isLoading) {
						play_pauseButton
								.setImageResource(R.mipmap.play_btn_play_big_detail);
					} else {
						play_pauseButton
								.setImageResource(R.mipmap.play_btn_play_big_detail_down);
					}
				} else {
					mMediaPlayerDelegate.start();
					if (null != play_pauseButton)
						if (!isLoading) {
							play_pauseButton
									.setImageResource(R.mipmap.play_btn_pause_big_detail);
						} else {
							play_pauseButton
									.setImageResource(R.mipmap.play_btn_pause_big_detail_down);
						}
		...
		if (null != mMediaPlayerDelegate
			...
			if (null != seekLoadingContainerView)
				seekLoadingContainerView
						.setBackgroundResource(R.mipmap.bg_play);
		}
...
//			Logger.d("sgh","plugin_small, from local: " + mMediaPlayerDelegate.videoInfo.getPlayType());
	...
	public void onPluginAdded() {
		...
		changePlayPause();
		if (mMediaPlayerDelegate.isFullScreen) {
			full_screenButton.setImageResource(R.mipmap.plugin_ad_gosmall);
		} else {
			full_screenButton
					.setImageResource(R.mipmap.detail_play_btn_full_screen);
		}
